...
SELECT_LATEST_POOL = """SELECT * FROM polls 
...
def get_latest_poll(connection):
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(SELECT_LATEST_POOL)
            return cursor.fetchall()
...
def create_poll(connection, title, owner, options):
    with connection:
        with connection.cursor() as cursor:
            ...
            values = [(option, pollid) for option in options]
            execute_values(connection, INSERT_OPTION, values)
